...
**PROGRESS 17/18 — tasks 1–17 done · task 18 whole-branch review pending**
...
- Branch: `feat/gui-live-console-wave2`, HEAD `f88a460`, stacked on `fix/ui-debt-wave1b-leftovers`.
- `git branch -vv` shows `f88a460 [origin/feat/gui-live-console-wave2: ahead 1]` — 1 local commit
  (`f88a460`, task 16 docs) not yet pushed.
- `git status --short` shows one untracked file: `packaging/wing-ui-debug.spec` (D-46, generated,
  not gitignored yet — do not `git add -A`).
...
Final, re-measured by me just now, at current HEAD `f88a460`:
```
...
- D10 (remembered addresses in `ui-state.json.consoles`, capped 8) — switching to e.g. QSettings
  replaces `state_store.normalize` (`state_store.py:27-42`) and the `remember_console` helper
  wholesale.
...
| Updating a PR body from a scratchpad file (`gh pr edit N --body-file <scratchpad>/...`) with a generic name risks two parallel sessions overwriting each other's draft if they land in a shared scratchpad. | Name the file per task or session, as task 15 did (`pr4-body-task15.md`) — not a bare `pr-body.md`. |
...
| `live_snapshot.py`, `live_events_view.py` and `live_controller.py` all sit at exactly 200 lines right now (verified with `wc -l` on all three). | The next change to any of them needs a split first, not a squeeze — there is no slack left. |

## 9. Verification commands

Run from this worktree root, snapshot at `f88a460`:
```
git log --oneline ac39cfa..HEAD | wc -l                     # 31
git status --short                                          # 1 untracked: packaging/wing-ui-debug.spec
git branch -vv | grep gui-live-console                      # ahead 1 of origin
